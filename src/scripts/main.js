$(document).ready(function() {
  ...
  $(".payment-method .r-tabs-accordion-title").on("click", "a", function() {
    $(".payment-method .r-tabs-accordion-title")
      .find("input")
      .prop("checked", false);
    $(this)
      .find("input")
  ...
  if ($(".productpage").length > 0) {
    $(window).bind("scroll", function() {
      if (
        $(window).scrollTop() >
        $(".product-list").offset().top - $("header").outerHeight()
      ) {
  ...
  if ($(".flash-news-slick .slick-prev")) {
    $(".flash-news-slick .slick-prev").insertBefore(
      ".flash-news-slick .slick-next"
    );
  }
  $(".picked-news").slick({
    slidesToShow: 4,
